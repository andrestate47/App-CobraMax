...
// ✅ Definimos el tipo local para auditoría (reemplaza al import fallido)
type TipoEntidad = "CLIENTE" | "PRESTAMO" | "PAGO" | "GASTO"
...
export async function PUT(
  ...
  try {
    ...
    const diasTotal = (diasPorTipo[tipoPago] || 1) * cuotasNum
  ...
  try {
    ...
    const prestamoExistente = await prisma.prestamo.findUnique({
      ...
      include: {
        pagos: true,
        cliente: {
          select: { nombre: true, apellido: true, codigoCliente: true }
        }
      }
    })
    ...
    // Registrar eliminación solo si no es administrador
    if (usuario.role !== "ADMINISTRADOR") {
      await registrarEliminacion(
        usuario.id,
        "PRESTAMO" as TipoEntidad,
        prestamoId,
        {
