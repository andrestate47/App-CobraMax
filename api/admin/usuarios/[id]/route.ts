...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      permissions: usuario.permissions.map((p: any) => p.permission),
...
export async function PUT(
  ...
  try {
    ...
    const usuarioExistente = await prisma.user.findUnique({ where: { id: params.id } })
    if (!usuarioExistente) {
    ...
    const usuarioActualizado = await prisma.user.findUnique({
      where: { id: params.id },
      include: { permissions: true, supervisor: true }
    })
    ...
    return NextResponse.json({
      ...
      permissions: usuarioActualizado!.permissions.map((p: any) => p.permission)
    })
...
export async function DELETE(
  ...
  try {
    ...
    const usuario = await prisma.user.findUnique({
      ...
      include: {
        _count: {
          select: { prestamos: true, pagos: true, gastos: true }
        }
      }
    })
    ...
    const tieneRegistros = usuario._count.prestamos > 0 || usuario._count.pagos > 0 || usuario._count.gastos > 0
    if (tieneRegistros) {
      return NextResponse.json(
