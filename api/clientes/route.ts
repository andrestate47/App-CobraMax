...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ Corrección: se tipa 'cliente' explícitamente
    return NextResponse.json(clientes.map((cliente: any) => ({
      id: cliente.id,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const clienteConMismoDocumento = await prisma.cliente.findUnique({ where: { documento } })
    if (clienteConMismoDocumento && clienteConMismoDocumento.id !== id) {
      return NextResponse.json({ error: "Ya existe otro cliente con este documento" }, { status: 400 })
    }

    if (codigoCliente && codigoCliente !== clienteExistente.codigoCliente) {
      const clienteConMismoCodigo = await prisma.cliente.findUnique({ where: { codigoCliente } })
      if (clienteConMismoCodigo && clienteConMismoCodigo.id !== id) {
        return NextResponse.json({ error: "Ya existe otro cliente con este código" }, { status: 400 })
      }
    }
