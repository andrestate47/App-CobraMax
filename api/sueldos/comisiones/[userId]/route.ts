...
export async function GET(
  ...
  try {
    ...
    const totalCobrado = pagos.reduce((sum: number, pago: any) => sum + parseFloat(pago.monto.toString()), 0)
...
const totalAvances = avancesOtorgados.reduce((sum: number, avance: any) => sum + parseFloat(avance.montoFinal.toString()), 0)
    ...
    const resumen = {
      ...
      detalleCobros: pagos.map((pago: any) => ({
