...
export async function POST(
  ...
  try {
    ...
    const totalPagado = prestamoAnterior.pagos.reduce(
    ...
    const dias =
      (diasPorCuota[tipoPago as keyof typeof diasPorCuota] || 1) * cuotasNuevas
    ...
    // ✅ Transacción Prisma con tipo correcto
    const resultado = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      await tx.prestamo.update({
        ...
        data: {
          ...
          observaciones: prestamoAnterior.observaciones
            ? `${prestamoAnterior.observaciones} | RENOVADO el ${new Date()
                .toISOString()
                .split("T")[0]}`
      ...
      // Si había saldo pendiente, registrar un pago automático
      if (saldoPendiente > 0) {
